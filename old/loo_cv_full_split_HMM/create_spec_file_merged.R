...
create_spec_file <- function(output_path, num_SNPs) {
    ...
    first_five <- round(seq(from = 0.1, to = 0.3, by = 0.05) * num_SNPs, digits = 0)
    second_five <- round(first_five + (first_five[2] - first_five[1])/2 + runif(1, -0.01*first_five[1], 0.01*first_five[1]), 0)
