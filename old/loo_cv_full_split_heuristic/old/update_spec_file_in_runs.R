...
# load SNPs to mask
#to_be_imputed <- as.numeric(read_lines(paste0(main_files, 'to_be_imputed_index.txt')))
...
create_cv_folder <- function(ind_num) {
    ...
    # if (dir.exists(run_dir)) stop("run directories already exist and have to be deleted")
    
    # copy AI, pedigree and spec file over
    #file.copy(from = paste0(main_files, "AlphaImputeLinux"), to =  run_dir,
    #          overwrite = TRUE)
    file.copy(from = paste0(main_files, "AlphaImputeSpec.txt"), to =  run_dir,
              overwrite = TRUE)
    #file.copy(from = paste0(main_files, "Pedigree.txt"), to = run_dir,
    #          overwrite = TRUE)
